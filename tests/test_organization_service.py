...
class TestOrganizationService:
    ...
    def test_find_similar_photos_success(self, org_service, sample_metadata_df):
        ...
        result = org_service.find_similar_photos(
            sample_metadata_df, similarity_threshold=0.5
        )
    ...
    def test_find_similar_photos_high_threshold(self, org_service, sample_metadata_df):
        ...
        result = org_service.find_similar_photos(
            sample_metadata_df, similarity_threshold=0.99
        )
    ...
    def test_organize_by_date_execute(
        self, org_service, sample_metadata_df, temp_photos_dir
    ):
    ...
    def test_organize_by_camera_execute(
        self, org_service, sample_metadata_df, temp_photos_dir
    ):
    ...
    def test_get_collection_stats_success(self, org_service, sample_metadata_df):
        ...
        assert stats["total_size_mb"] == pytest.approx(
            7.0 / 1024, rel=1e-2
        )  # 7168 bytes
    ...
    def test_clean_folder_name(self, org_service):
        ...
        test_cases = [
            ...
            (
                ...
                "File@with#special$chars",
            ),  # These chars are not in invalid_chars
        ]
    ...
    @patch("modules.services.organization_service.shutil.move")
    def test_organize_by_date_file_move_error(
        self, mock_move, org_service, sample_metadata_df, temp_photos_dir
    ):
    ...
    def test_cluster_photos_different_cluster_counts(
        self, org_service, sample_metadata_df
    ):
        """Test clustering with different numbers of clusters."""
        for n_clusters in [1, 2, 3, 4]:
            result = org_service.cluster_photos(
                sample_metadata_df, n_clusters=n_clusters
            )
            assert "error" not in result
            assert len(result) == n_clusters

    def test_find_similar_photos_different_thresholds(
        self, org_service, sample_metadata_df
    ):
